...
if (!empty($search)) {
    ...
    if (empty($filtered_items)) {
        $filtered_items = binarySearchItems($orders_array, $search, ['name']);
        // Re-apply date filter if needed
        if ($start_date && $end_date && !$error) {
            $filtered_items = array_filter($filtered_items, function($item) use ($start_date, $end_date) {
                $created_at = substr($item['created_at'], 0, 10);
                return $created_at >= $start_date && $created_at <= $end_date;
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-primary">
                    <h4 class="text-white">Out For Delivery History
                        <a href="orders.php" class="btn btn-warning float-end btn-sm">Back</a>
                    </h4>
                </div>
                <div class="card-body" id="orders_table">
                    ...
                    <div class="date-range-form">
                        <form method="GET" class="row g-2 align-items-center">
                            ...
                            <div class="col-md-2">
                                <button type="submit" class="btn btn-primary btn-lg w-100"><i class="fas fa-filter me-2"></i>Filter</button>
                            </div>
                            <div class="col-md-2">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            if (!empty($filtered_items)) {
                                ...
                                echo "<tr><td colspan='7'>No Out Of Delivery Orders Found</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
